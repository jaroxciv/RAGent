...
from utils import (
    call_claude_rag,
)  # Removed assess_confidence, synthesize_information, call_tavily_web_search
import logging
import os
...
def main(
    user_query, initialize=False, directory_path=None, dry_run=False
):  # directory_path can be kept for flexibility or future use
    if user_query:
        logging.debug(f"[main] Starting process with query: {user_query[:100]}...")
    else:
        logging.debug(
            "[main] Starting process (no user query, likely initialization or internal call)..."
        )
    ...
    if initialize:
        ...
        logging.info(
            f"[main] Attempting initialization/update from preset directory: {PDF_DIRECTORY}"
        )
        if not os.path.exists(PDF_DIRECTORY):
            os.makedirs(
                PDF_DIRECTORY, exist_ok=True
        ...
        if vector_store.initialize_from_directory(PDF_DIRECTORY):
            ...
            return (
                f"Error processing PDFs from '{PDF_DIRECTORY}'. Check logs for details."
            )
    ...
    if not vector_store.db and not initialize:
        ...
        initialize
    ):  # This means initialize is True but directory_path was not provided
