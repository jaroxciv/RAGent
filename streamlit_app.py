...
def main_chat():
    ...
    with st.sidebar:
        st.title("Settings")
        dry_run = st.checkbox(
            "🔧 Dry Run Mode", value=False, help="Run without making API calls"
        )
        if dry_run:
    ...
    st.session_state.dry_run_mode = (
        dry_run  # Store dry_run_mode in session state from sidebar
    )
    ...
    if not st.session_state.get("initialized", False):  # Use .get for safety
        ...
        pdf_dir_path_display = main.__globals__.get(
            "PDF_DIRECTORY", "./podcast_pdfs/"
        )  # Default if not found

        with st.spinner(
            f"Checking and initializing from preset PDF directory ({pdf_dir_path_display})..."
        ):
            # Call main with initialize=True. main will use its PDF_DIRECTORY.
            # Pass the current dry_run state from the session.
            init_response = main(
                user_query=None, initialize=True, dry_run=st.session_state.dry_run_mode
            )

        # Check specific phrases that indicate successful processing and readiness
        if (
            "Successfully processed" in init_response
            and "Ready for questions." in init_response
        ):
    ...
    if st.session_state.get("initialized", False):
        ...
        if prompt := st.chat_input("What would you like to know about the podcasts?"):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    ...
                    response = main(
                        prompt, initialize=False, dry_run=st.session_state.dry_run_mode
                    )
        ...
        st.markdown("---")
        st.info(
            f"Podcast library not yet ready. Please ensure PDF files are in the `podcast_pdfs` directory. Refresh the page to re-attempt initialization."
        )
